...
AUDIO_GAIN         = 2.0      # 오디오 증폭 배수 (1.0=원본, 2.0=2배 증폭, 3.0=3배, 4.0=4배, 5.0=5배)
SILENCE_MULTIPLIER = 1.2      # 노이즈 플로어 대비 이 배수 이상이면 음성으로 판단 (낮을수록 민감, 1.0=노이즈와 동일)
MIN_RMS_THRESHOLD  = 0.0001   # RMS 최소 임계값 (이 아래는 무조건 무음) - 더 작게하여 작은 소리도 감지
...
def main():
    ...
    console.print(
        Panel(
            ...
            "[dim]로컬 Whisper + Ollama/Minimax 기반 실시간 번역[/dim]",
            border_style="cyan",
            padding=(1, 4),
        )
    )
